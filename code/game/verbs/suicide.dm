...
/mob/living/proc/be_suicidal(forced = FALSE)
	...
	if(forced || (confirm == "Да"))
		if(!forced && isAntag(src))
			confirm = tgui_alert(src, "Вы абсолютно уверены в этом? Беспричинные самоубийства, а так же самоубийства после задержания или получения роли антагониста, могут карается баном!", "Подтвердить самоубийство", list("Да", "Нет"))
			if(confirm == "Да")
				suiciding = TRUE
				do_suicide()
